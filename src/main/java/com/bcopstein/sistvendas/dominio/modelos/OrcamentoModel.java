...
public class OrcamentoModel {
    ...
    public BigDecimal getAliquotaImpostoEstadualAplicada() { return aliquotaImpostoEstadualAplicada; } // Geralmente não é arredondada
    ...
    public void recalculaTotais() {
        ...
        if (this.itens != null) {
            for (ItemPedidoModel item : this.itens) {
                if (item.getProduto() != null && item.getProduto().getPrecoUnitario() >= 0 && item.getQuantidade() >= 0) {
    ...
    private void calcularImpostoEstadualInterno() {
        ...
        switch (estadoUpper) {
            case "RS": // Rio Grande do Sul [cite: 81]
                // Orçamentos de menos de R$ 100,00 são isentos. [cite: 81]
                // Para orçamentos maiores que R$ 100,00 a alíquota é de 10% sobre o valor que ultrapassa R$ 100,00. [cite: 81]
                if (this.custoItens.compareTo(cem) >= 0) {
                    this.aliquotaImpostoEstadualAplicada = new BigDecimal("0.10");
                    this.impostoEstadual = (this.custoItens.subtract(cem)).multiply(this.aliquotaImpostoEstadualAplicada);
                } else {
                    this.impostoEstadual = BigDecimal.ZERO; // Isento
                    this.aliquotaImpostoEstadualAplicada = BigDecimal.ZERO;
                }
                break;
            case "SP": // São Paulo [cite: 81]
                // Alíquota única de 12%. [cite: 81]
                ...
                break;
            case "PE": // Pernambuco [cite: 81]
                // Alíquota única de 15% sobre todos os produtos menos aqueles considerados essenciais que tem uma alíquota de 5%. [cite: 81]
                // Produtos essenciais tem um "*" ao final da descrição do produto. [cite: 81]
                ...
                if (this.itens != null) {
                    for (ItemPedidoModel item : this.itens) {
                        ...
                        if (produto.getDescricao().endsWith("*")) { // Produto essencial
                            ...
                            impostoNaoEssenciais = impostoNaoEssenciais.add(valorItem.multiply(aliquotaNaoEssenciaisPE));
                        }
                    }
                }
                ...
                if (this.custoItens.compareTo(BigDecimal.ZERO) > 0) {
                    this.aliquotaImpostoEstadualAplicada = this.impostoEstadual.divide(this.custoItens, 4, RoundingMode.HALF_UP);
                } else {
            ...
            default:
                ...
                this.impostoEstadual = BigDecimal.ZERO;
                this.aliquotaImpostoEstadualAplicada = BigDecimal.ZERO;
                // Idealmente: throw new IllegalArgumentException("Estado '" + this.estadoCliente + "' não atendido ou sem regra de imposto definida.");
                break;
        }
...
private void calcularDescontosInterno() {
    ...
    if (this.itens != null && this.itens.size() > 10) {
        System.out.println("DEBUG: CONDIÇÃO DE DESCONTO POR VOLUME ATENDIDA. CustoItens: " + this.custoItens.toPlainString());
        ...
        System.out.println("DEBUG: Desconto por volume: " + descontoPorVolume.toPlainString() + ", Desconto Acumulado Total: " + descontoCalculado.toPlainString());
    } else {
        System.out.println("DEBUG: CONDIÇÃO DE DESCONTO POR VOLUME NÃO ATENDIDA.");
    }
    
    this.valorDesconto = descontoCalculado;
    System.out.println("DEBUG: Saindo de calcularDescontosInterno(). ValorDesconto FINAL: " + this.valorDesconto.toPlainString());
}
    ...
    public String toString() {
        return "OrcamentoModel{" +
                ...
                " (aliquotaAplicada=" + (aliquotaImpostoEstadualAplicada != null ? aliquotaImpostoEstadualAplicada.multiply(new BigDecimal("100")).setScale(2, RoundingMode.HALF_UP).toPlainString() + "%" : "0.00%") + ")" +
                ", impostoFederal=" + (impostoFederal != null ? impostoFederal.setScale(2, RoundingMode.HALF_UP).toPlainString() : "0.00") + " (15.00%)" + // Imposto Federal é 15% [cite: 82]
                ", valorDesconto=" + (valorDesconto != null ? valorDesconto.setScale(2, RoundingMode.HALF_UP).toPlainString() : "0.00") +
                ", custoConsumidor=" + (custoConsumidor != null ? custoConsumidor.setScale(2, RoundingMode.HALF_UP).toPlainString() : "0.00") +
                ", efetivado=" + efetivado +
                '}';
    }
}